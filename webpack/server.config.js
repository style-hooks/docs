...
if (isDev || stage === 'development') {
  ...
  envConfig = {
    plugins: [
      ...
      // Copies the service worker from the client bundle to the server dist
      // new CopyPlugin([
      //   {
      //     from: path.join(paths.dist, stage, 'client', 'sw.js'),
      //     to: path.join(paths.dist, stage, 'server', 'sw.js')
      //   },
      // ])
    ]
  }
